...
把鼠标移到桌宠上，右上角会浮现一个像素「收纳」按钮（也可以用菜单栏的 Collapse / Expand）。点它就把桌宠收进一个 56×56 的像素泡泡，配色取自桌宠主色（`pet.json` 里的 `accentColor`，默认青绿），泡泡里嵌一张待机缩略图。拖动泡泡可移动，点一下把桌宠展开回来。收起状态与两处位置都会记住。

泡泡右上角有一个红色徽标，数字表示当前有几个 Claude Code 窗口正在 `waiting`（等你处理）。这样即使桌宠收起来了，也能一眼看到有 N 个会话需要你。实现上，每个会话各自写一份 `~/.claude/pet/sessions/<session_id>.state.json`；app 扫描这些文件，用最近活跃的那个会话作为桌宠显示状态，并统计其中处于 waiting 的数量作为徽标。窗口关闭时触发的 `SessionEnd`（或超过 TTL 的过期会话）会被剔除，不再计入。
